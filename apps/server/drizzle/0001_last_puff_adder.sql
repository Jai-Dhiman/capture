...
ALTER TABLE `profile` RENAME COLUMN "name" TO "username";--> statement-breakpoint
ALTER TABLE `profile` RENAME COLUMN "media_verified_type" TO "verified_type";--> statement-breakpoint
...
CREATE TABLE `__new_profile` (
	...
	`created_at` numeric DEFAULT '2025-01-31T01:40:29.993Z' NOT NULL,
	`updated_at` numeric DEFAULT '2025-01-31T01:40:29.999Z' NOT NULL
);
--> statement-breakpoint
INSERT INTO `__new_profile`("id", "username", "email", "phone_number", "image", "bio", "verified_type", "created_at", "updated_at") SELECT "id", "username", "email", "phone_number", "image", "bio", "verified_type", "created_at", "updated_at" FROM `profile`;--> statement-breakpoint
...
CREATE INDEX `email_idx` ON `profile` (`email`);--> statement-breakpoint
CREATE TABLE `__new_comment` (
	...
	`created_at` numeric DEFAULT '2025-01-31T01:40:30.001Z' NOT NULL,
...
CREATE TABLE `__new_post` (
	...
	`user_id` text,
	`content` text NOT NULL,
	`created_at` numeric DEFAULT '2025-01-31T01:40:30.000Z' NOT NULL,
	FOREIGN KEY (`user_id`) REFERENCES `profile`(`id`) ON UPDATE no action ON DELETE no action
);
--> statement-breakpoint
...
CREATE TABLE `__new_relationship` (
	...
	`followed_id` text,
	`created_at` numeric DEFAULT '2025-01-31T01:40:30.001Z' NOT NULL,
	FOREIGN KEY (`follower_id`) REFERENCES `profile`(`id`) ON UPDATE no action ON DELETE no action,
	FOREIGN KEY (`followed_id`) REFERENCES `profile`(`id`) ON UPDATE no action ON DELETE no action
);
--> statement-breakpoint
...
CREATE INDEX `post_captag_composite_idx` ON `post_captag` (`post_id`,`hashtag_id`);
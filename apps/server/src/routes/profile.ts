...
router.get("/check-username", async (c) => {
  ...
  const existingProfile = await db.select().from(profile).where(eq(profile.username, username)).get();
...
router.post("/", async (c) => {
  ...
  try {
    ...
    const existingProfile = await db.select().from(profile).where(eq(profile.username, data.username)).get();

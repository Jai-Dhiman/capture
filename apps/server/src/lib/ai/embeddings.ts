import type { QdrantClient } from '../infrastructure/qdrantClient';
import { createVoyageService } from './voyageService';
...
export async function generateEmbedding(text: string, env: Bindings, retries = 2): Promise<number[]> {
  try {
    const cache = createCachingService(env);
    const voyageService = createVoyageService(env, cache);
    
    return await voyageService.generateTextEmbedding(text);
  } catch (error) {
    if (retries > 0) {
      console.warn(`Embedding generation failed, retrying (${retries} retries left)`, error);
      await new Promise((resolve) => setTimeout(resolve, 500));
      return generateEmbedding(text, env, retries - 1);
    }
...
export async function storePostEmbedding(
  ...
  try {
    ...
    // Validate expected embedding dimension (Voyage multimodal)
